...
function endEditSongName(index) {
	...
	if (el != null) {
		...
		xhttp.open('GET', 'edit?i='+index+'&v='+txt, true);
		xhttp.send();
	}
	return false;
}
...
function add() {
	...
	xhttp.open('GET', 'add?songId='+songId, true);
	xhttp.send();
...
function listAdd() {
	...
	xhttp.open('GET', 'listadd?songId='+songId, true);
	xhttp.send();
...
function remove(i) {
	...
	xhttp.open('GET', 'remove?idx='+i, true);
	xhttp.send();
}
...
function moveUp(i) {
	if (i != 0) {
		...
		xhttp.open('GET', 'move?i='+i+'&j='+(i-1), true);
		xhttp.send();
	}
}
...
function moveToTop(i) {
	if (i != 0) {
		...
		xhttp.open('GET', 'move?i='+i+'&j=0', true);
		xhttp.send();
	}
}
...
function moveToBottom(i) {
	if (i != playlistSize-1) {
		...
		xhttp.open('GET', 'move?i='+i+'&j='+(playlistSize-1), true);
		xhttp.send();
	}
}
...
function voteSkip() {
	...
	if (reason != null) {
		...
		if (reason != '') {
			xhttp.open('GET', 'skip?reason='+reason, true);
		}
		else {
			xhttp.open('GET', 'skip?reason=null', true);
		}
		xhttp.send();
	}
	else {
